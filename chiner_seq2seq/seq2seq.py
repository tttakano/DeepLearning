...
class Seq2seq(chainer.Chain):
    ...
    def __call__(self, xs, ys):
        ...
        hx, cx, _ = self.encoder(None, None, exs)           #hidden_state, p_param, encoder(hiddenn_state, preb_c, words)
        _, _, os = self.decoder(hx, cx, eys)                     #####todo?
...
class CalculateBleu(chainer.training.Extension):
    ...
    def __init__(
            self, model, test_data, key, batch=100, device=-1, max_length=100):             #todo change batch size 
        self.model = model
...
def main():
    ...
    train_source = load_data(source_ids, args.SOURCE)        #[i, am, from, japan]  →    [123, 345, 456, 567]
    train_target = load_data(target_ids, args.TARGET)
    assert len(train_source) == len(train_target)
    train_data = [(s, t)                                                  #cut too long sentence
    ...
    train_source_unknown = calculate_unknown_ratio(          #calculate
